...
def drop_rows_with_high_missing_values(group):
    print('processing 2/2 ' + str(index) + ' of ' + str(job_length))
    # Calculate the percentage of missing values for each column
    percentage_of_missing_values = group.isnull().mean()
 
    # Drop df with more than 20% missing values
    if percentage_of_missing_values.mean() > 0.2:
            print('culled dataframe NaN - ' + str(percentage_of_missing_values))
...
#apply the zero percentage rule function to the array of DFs 
index = 0
for x in veh_day_list:
    veh_day_list[index] = drop_columns_with_high_zero_percentage(x)
    index += 1
...
output_df.to_csv("C:/Users/space/Desktop/ULB docs/---Data Mining/project/cleaned_stage1.csv", sep = ';')
